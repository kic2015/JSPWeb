...
<%
	...
	// 2. 세션의 cart라는 이름의  속성을 얻어온다. 
	Object obj = session.getAttribute("cart");//세션.속성얻다(getAttribute) cart라는 이름의

	// 3. 만일 null(cart)이면 ArrayList 객체 새로 생성하고 그렇지 않으면 ArrayList 변수(glist)에 지정
	if (obj == null) {
		glist = new ArrayList<>(); //ArrayList 객체 새로 생성해서 glist에 담아준다	
	} else {
		glist = (ArrayList<Goods>) obj;
	}
	// 4. 1번의 값들을 Goods 객체로 생성후  ArrayList 에 추가
	Goods gs = new Goods(id, name, price);//해당 값들을 goods로 객체로 생성한후
	glist.add(gs);//ArrayList에 추가
...
<body bgcolor=white>
	...
	<table>
		...
		%>
		<tr bgcolor="#e7a068">
			<td colspan="2" align="right">총액 : <%=sum%></td>
		</tr>
	</table>
	<br />
	<br /> [
